...
class ChatTree {
    ...
    async getMessage(input){
        ...
        if(this.firstMsg===true) {
            ...
            let x=this.chat_tree['children'].length;
            if(isNaN(parseInt(input)) || parseInt(input)<=0 || parseInt(input) > x+1)
        ...
        if("message" in this.chat_tree){
            ...
            if(this.chat_tree['type']==="function"){
                ...
                if(this.chat_tree['message']==="getJoke()"){
                    ...
                    let random_joke_index=Math.floor(Math.random()*(100-1+1))+1;
                    console.log('Random joke index betwen 1 and 100:',random_joke_index)
                    data = data.value[random_joke_index].joke;
                }
                else if(this.chat_tree['message']==="getquote()"){
                    data = await eval(this.chat_tree['message']);
                    let random_quote_index=Math.floor(Math.random()*(100-1+1))+1;
                    console.log(random_quote_index);
                    data=data[random_quote_index].text
                    
                }  
                else{

                    data = await eval(this.chat_tree['message']);
                    data = data.results[1].title;
                }
            } else{
...
async function getNews(num) {
    category =["world","arts","science"]
    const response = await fetch('https://api.nytimes.com/svc/topstories/v2/'+category[num]+'.json?api-key=X39k1ZBJGAnAC5121YUoSpTUGytN6Glk');
    const jsonResp = await response.json();
    return jsonResp;
}
